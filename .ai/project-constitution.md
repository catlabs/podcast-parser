...
| 9 | **deferred** | **Retrieval stack migration: MiniLM + Chroma → Azure OpenAI embeddings + Azure AI Search.** Re-introduced later as an agent-tool upgrade (see 2026-06-06 recalibration). Must include an **explicit comparison phase** before committing: quality (semantic relevance on this corpus), latency (local inference vs. API round-trip), cost (embedding API + AI Search query-unit pricing), observability (span coverage at the AI Search tier), operational complexity (managed vs. self-hosted Chroma), and container image size (MiniLM baked into the container is currently 2+ GB; AI Search removes that layer entirely). The comparison phase can be run ad-hoc today, or rigorously + repeatably via the **Step 9b** retrieval-profile harness once that exists. |
| 9b | **future (low-prio)** | **Retrieval-profile abstraction — calibrate & benchmark retrieval backends consistently.** A `RetrievalProfile` / adapter layer behind a **common retrieval contract** (query → ranked results) that is *independent of the backend* (Chroma + local embeddings, Azure AI Search + Azure OpenAI embeddings, future providers), so backends are swappable and directly comparable. Scope: (a) the **adapter layer + common contract** decoupling agents/eval from any one store; (b) **per-provider score normalization** — each backend returns a different score scale (Chroma squared-L2 → derived cosine today; AI Search returns its own `@search.score`), normalized to a comparable `[0,1]` per profile; (c) **per-provider threshold calibration** — the `min_score` tuned for minilm (~0.45) does NOT transfer across backends, so calibrate per profile; (d) **golden-dataset comparison across providers** — generalize the Eval.1 dataset + per-model baselines (`rag/eval_baselines/<key>.json`) into per-profile baselines; (e) **benchmark quality + latency + cost** side-by-side across providers; (f) the common contract makes the SearchAgent + eval gate backend-agnostic. Enables Step 9's mandatory comparison to be principled rather than ad-hoc. **Not on the pre-mission-start critical path** — a post-sprint / nice-to-have hardening item; pick up only when retrieval-backend choice becomes a live question. |
| 10 | — | Azure Speech |
